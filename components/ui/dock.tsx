...
function DockItem({ children, className }: DockItemProps) {
  ...
  const widthTransformBase = useTransform(mouseDistance, [-distance, 0, distance], [40, magnification, 40])

  const widthTransform = useMemo(() => widthTransformBase, [widthTransformBase])
...
function DockLabel({ children, className, ...rest }: DockLabelProps) {
  ...
  useEffect(() => {
    // Only set up the subscription if isHovered is defined
    if (isHovered) {
    ...
    // If isHovered is undefined, we don't need to clean up anything
    return undefined
  }, [isHovered])
...
function DockIcon({ children, className, ...rest }: DockIconProps) {
  ...
  // Use the width if it exists, otherwise use the default
  const actualWidth = width || defaultWidth

  // Now we can safely use useTransform
